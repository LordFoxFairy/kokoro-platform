...
type OpenAiCompatibleAdapterInput = Readonly<{
  endpoint: string;
  apiKey: string;
  fetch?: FetchPort;
  timeoutMs?: number;
...
class OpenAiCompatibleChatAdapter implements ModelGatewayProviderPort {
  readonly #endpoint: string;
  readonly #apiKey: string;
  readonly #fetch: FetchPort;
  readonly #timeoutMs: number;
  readonly #clock: () => Date;
  readonly #transportKind: "litellm" | "direct";
  readonly #forwardProviderOperationHeader: boolean;

  constructor(input: OpenAiCompatibleAdapterInput & Readonly<{
    transportKind: "litellm" | "direct";
    forwardProviderOperationHeader: boolean;
  }>) {
    const errorPrefix = `MODEL_GATEWAY_${input.transportKind.toUpperCase()}`;
    this.#endpoint = endpoint(input.endpoint, `${errorPrefix}_ENDPOINT_INVALID`);
    if (
      input.apiKey.length < 1 || input.apiKey.length > 4_096 ||
      hasControlCharacter(input.apiKey)
    ) throw new Error(`${errorPrefix}_KEY_INVALID`);
    ...
    if (!Number.isInteger(this.#timeoutMs) || this.#timeoutMs < 100 || this.#timeoutMs > 120_000) {
      throw new Error(`${errorPrefix}_TIMEOUT_INVALID`);
    }
    this.#clock = input.clock ?? (() => new Date());
    this.#transportKind = input.transportKind;
    this.#forwardProviderOperationHeader = input.forwardProviderOperationHeader;
  }
  ...
  prepare(
    ...
    if (
      authorization.adapterKind !== this.#transportKind ||
      request.model !== authorization.authorizedGatewayModel
    ) {
      throw new Error("MODEL_GATEWAY_AUTHORIZATION_ROUTE_MISMATCH");
    }
    const providerModel = this.#transportKind === "direct"
      ? authorization.providerModel
      : authorization.authorizedGatewayModel;
    if (!safeName(providerModel)) throw new Error("MODEL_GATEWAY_AUTHORIZATION_ROUTE_INVALID");
  ...
  async *#stream(
    ...
    try {
      response = await this.#fetch(this.#endpoint, {
        ...
        headers: {
          ...
          ...(this.#forwardProviderOperationHeader
            ? { "x-kokoro-provider-operation-key": input.providerOperationKey }
            : {}),
        },
      ...
      yield Object.freeze({
        ...
        outcome: unknownOutcome(`${this.#transportKind}-transport`, errorNameDigest(error)),
      });
      return;
    }
      ...
      try {
        ...
        yield Object.freeze({
          ...
          outcome: unknownOutcome(
            `${this.#transportKind}-response`,
            sha256(`${response.status}:body-unavailable`),
          ),
        });
        return;
      }
    ...
    if (!(response.headers.get("content-type") ?? "").toLowerCase().startsWith("text/event-stream")) {
      yield Object.freeze({
        kind: "terminal" as const,
        outcome: unknownOutcome(`${this.#transportKind}-response`, sha256("content-type-invalid")),
      });
      return;
    }
    ...
    try {
      for await (const data of readBoundedSse(response, streamState)) {
        if (data === "[DONE]") {
          if (aggregate.done) throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_MULTIPLE_TERMINALS");
          aggregate.done = true;
          continue;
        }
        if (aggregate.done) throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_DATA_AFTER_TERMINAL");
      ...
      if (!aggregate.done || completed === null) {
        yield Object.freeze({
          kind: "terminal" as const,
          outcome: unknownOutcome(`${this.#transportKind}-stream`, sourceDigest),
        });
        return;
      }
      yield Object.freeze({
      ...
      yield Object.freeze({
        kind: "terminal" as const,
        outcome: unknownOutcome(`${this.#transportKind}-stream`, sha256(
  ...
  #now(): string {
    ...
    if (!Number.isFinite(value.getTime())) {
      throw new Error(`MODEL_GATEWAY_${this.#transportKind.toUpperCase()}_CLOCK_INVALID`);
    }
    return value.toISOString();
  }
}

/** One authorized Gateway alias maps to one LiteLLM alias. */
export class LiteLlmChatAdapter extends OpenAiCompatibleChatAdapter {
  constructor(input: OpenAiCompatibleAdapterInput) {
    super({ ...input, transportKind: "litellm", forwardProviderOperationHeader: true });
  }
}

/** One Gateway alias maps to one certified OpenAI-compatible provider model. */
export class DirectOpenAiChatAdapter extends OpenAiCompatibleChatAdapter {
  constructor(input: OpenAiCompatibleAdapterInput) {
    super({ ...input, transportKind: "direct", forwardProviderOperationHeader: false });
  }
}
...
function parseStreamChunk(
  ...
  let value: unknown;
  try { value = JSON.parse(data); } catch { throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_JSON_INVALID"); }
  if (!record(value)) throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_CHUNK_INVALID");
  if (value.id !== undefined) {
    if (!safeReference(value.id) || (aggregate.id !== undefined && aggregate.id !== value.id)) {
      throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_CHUNK_INVALID");
    }
    aggregate.id = value.id;
  }
  if (value.usage !== undefined && value.usage !== null) {
    if (!record(value.usage)) throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_USAGE_INVALID");
    ...
    if (input === null || output === null || input > BigInt(Number.MAX_SAFE_INTEGER) ||
      ...
      throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_USAGE_INVALID");
    }
    aggregate.usage = Object.freeze([
      Object.freeze({ dimensionKey: "input_tokens", sourceUnit: TOKEN_SOURCE_UNIT, quantity: input }),
      Object.freeze({ dimensionKey: "output_tokens", sourceUnit: TOKEN_SOURCE_UNIT, quantity: output }),
    ]);
  ...
  if (!Array.isArray(value.choices) || value.choices.length > 1) {
    throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_CHOICES_INVALID");
  }
  if (value.choices.length === 0) return Object.freeze([]);
  const choice = value.choices[0];
  if (!record(choice) || choice.index !== 0 || !record(choice.delta)) {
    throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_CHOICE_INVALID");
  }
  const result: ModelGatewayProviderStreamEvent[] = [];
  const content = choice.delta.content;
  if (content !== undefined && content !== null) {
    if (typeof content !== "string") throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_CONTENT_INVALID");
    ...
    if (aggregate.totalOutputBytes > MAX_RESPONSE_BYTES) {
      throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_RESPONSE_TOO_LARGE");
    }
  ...
  if (reasoning !== undefined && reasoning !== null) {
    if (typeof reasoning !== "string") throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_REASONING_INVALID");
  ...
  if (toolCalls !== undefined) {
    if (!Array.isArray(toolCalls) || toolCalls.length > 128) {
      throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_TOOL_INVALID");
    }
    for (const raw of toolCalls) {
      if (!record(raw) || !Number.isInteger(raw.index) || (raw.index as number) < 0 ||
          (raw.index as number) > 127) throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_TOOL_INVALID");
      ...
      if (raw.function !== undefined) {
        if (!record(raw.function)) throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_TOOL_INVALID");
        ...
        if (raw.function.arguments !== undefined) {
          ...
          if (Buffer.byteLength(current.arguments, "utf8") > 1024 * 1024) {
            throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_TOOL_TOO_LARGE");
          }
        }
      }
  ...
  if (choice.finish_reason !== undefined && choice.finish_reason !== null) {
    if (typeof choice.finish_reason !== "string" || choice.finish_reason.length < 1 ||
        choice.finish_reason.length > 64) throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_FINISH_INVALID");
...
async function* readBoundedSse(
  ...
  if (response.body === null) throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_EMPTY");
  ...
  try {
    while (true) {
      ...
      if (state.totalBytes > 16 * 1024 * 1024) throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_TOO_LARGE");
      state.hash.update(item.value);
      buffered += decoder.decode(item.value, { stream: true });
      if (Buffer.byteLength(buffered, "utf8") > 1024 * 1024) {
        throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_EVENT_TOO_LARGE");
      }
      while (true) {
        ...
        if (line.length === 0) {
          ...
          if (field !== "event" && field !== "id" && field !== "retry") {
            throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_FIELD_INVALID");
          }
        }
      }
    }
    ...
    if (buffered.length > 0) {
      ...
      if (line.startsWith("data:")) {
        const data = line.slice(5);
        dataLines.push(data.startsWith(" ") ? data.slice(1) : data);
      } else if (line.length > 0) throw new Error("MODEL_GATEWAY_OPENAI_COMPATIBLE_SSE_TRUNCATED");
    }
...
function endpoint(value: string, code = "MODEL_GATEWAY_LITELLM_ENDPOINT_INVALID"): string {
  let parsed: URL;
  try {
