...
import { DurableSessionAuthorizationPublisher } from "../modules/authorization/application/services/publish-session-authorization.js";
import {
...
export async function createPlatformPublicProductionComposition(input: Readonly<{
  ...
  const [workloads, keyRing, tls] = await Promise.all([
    loadProductWorkloadRegistry(required(environment, "PLATFORM_PRODUCT_WORKLOAD_REGISTRY_FILE")),
    loadKeyRing(required(environment, "PLATFORM_SESSION_ACCESS_KEY_RING_FILE")),
    loadTls(environment),
  ]);
...
async function loadTls(environment: Readonly<Record<string, string | undefined>>): Promise<ServerOptions> {
  const [key, cert, ca] = await Promise.all([
    readBounded(required(environment, "PLATFORM_PUBLIC_TLS_KEY_FILE"), 64 * 1024),
    readBounded(required(environment, "PLATFORM_PUBLIC_TLS_CERT_FILE"), 64 * 1024),
    readBounded(required(environment, "PLATFORM_PUBLIC_TLS_CLIENT_CA_FILE"), 256 * 1024),
  ]);
...
async function loadKeyRing(path: string): Promise<SessionAccessKeyRingConfig> {
  const parsed = JSON.parse(await readBounded(path, 512 * 1024)) as unknown;
  const root = record(parsed, "SESSION_ACCESS_KEY_RING_INVALID");
  exact(root, ["version", "issuer", "maximumTtlSeconds", "keys"]);
  if (root.version !== 1 || !Array.isArray(root.keys)) throw new Error("SESSION_ACCESS_KEY_RING_INVALID");
  const keys = root.keys.map((raw): SessionAccessSigningKeyConfig => {
    const key = record(raw, "SESSION_ACCESS_KEY_RING_INVALID");
    exact(key, ["keyRevision", "publicKeyPem", "privateKeyPem", "current"]);
    if (
