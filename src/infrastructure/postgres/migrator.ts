...
import {
  finalizeAdmissionRoleLease,
  prepareAdmissionRoleLease,
...
export async function runPlatformMigrations(
  ...
  try {
    ...
    const admissionLeaseInput = Object.freeze({
      targetRole: admissionRole,
      migratorRole: config.expectedDatabaseUser,
      databaseName: config.expectedDatabaseName,
    });
    await prepareAdmissionRoleLease(lockClient, admissionLeaseInput);

    await finalizeAdmissionRoleLease(lockClient, admissionLeaseInput, {
  ...
  const result = await client.query(
    `SELECT EXISTS (
       ...
       WHERE authority.role_kind='admission'
         ...
         AND authority.pending_role_name IS NULL
         AND authority.pending_role_oid IS NULL
         AND authority.retiring_role_names='{}'::text[]
         AND authority.retiring_role_oids='{}'::bigint[]
...
async function grantModelGatewayPrivileges(
  ...
  await client.query(
    `GRANT SELECT(${ADMISSION_MODEL_GATEWAY_SELECT_COLUMNS.join(",")}), ` +
      `UPDATE(${ADMISSION_MODEL_GATEWAY_UPDATE_COLUMNS.join(",")}) ` +
...
const POST_MIGRATION_AUTHORITY_SQL = `
  SELECT runtime_role.rolname AS "roleName",
         ...
         ,CASE WHEN runtime_role.rolname = $1 THEN
           ...
           AND (has_table_privilege(runtime_role.rolname, 'platform.commerce_credit_program_window_acquisition', 'SELECT') AND has_table_privilege(runtime_role.rolname, 'platform.commerce_credit_program_window_acquisition', 'INSERT'))
         ...
         ,CASE WHEN runtime_role.rolname=$5 THEN
            has_table_privilege(
              runtime_role.rolname,'platform.model_gateway_execution_authorization','INSERT'
            )
            AND NOT has_table_privilege(
              runtime_role.rolname,'platform.model_gateway_execution_authorization','SELECT'
            )
            AND NOT has_table_privilege(
              runtime_role.rolname,'platform.model_gateway_execution_authorization','UPDATE'
            )
            AND (
              SELECT count(*) FROM unnest(ARRAY[${ADMISSION_MODEL_GATEWAY_SELECT_COLUMNS_SQL}])
                AS expected(column_name)
               WHERE has_column_privilege(
                 runtime_role.rolname,'platform.model_gateway_execution_authorization',
              ...
              SELECT 1 FROM pg_attribute candidate_column
               WHERE candidate_column.attrelid=
                 ...
                 AND has_column_privilege(
                   runtime_role.rolname,candidate_column.attrelid,candidate_column.attnum,'SELECT'
                 )
            )
            AND (
            ...
            AND NOT EXISTS (
              SELECT 1 FROM pg_attribute candidate_column
               WHERE candidate_column.attrelid=
                 ...
                 AND has_column_privilege(
                   runtime_role.rolname,candidate_column.attrelid,candidate_column.attnum,'UPDATE'
                 )
            )
