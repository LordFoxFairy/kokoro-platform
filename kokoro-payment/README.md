...
Repository gate 固定完整 HTTP 源文件清单、从真实 `main.ts` 启动入口计算的 runtime import graph、Catalog/Admin 各自精确的只读端口、每个可达生产文件的直接 import，以及 dedicated read credential、deployment template 和 catalogue seed。生产依赖限制和代码执行逃逸由 ESLint 的 `no-restricted-imports`、`no-eval`、`no-implied-eval`、`no-new-func` 承重；Prisma adapter 只保留针对本边界的结构检查，禁止 computed access、client/delegate alias 和非白名单调用。Fastify `onRoute` 在真实 production/non-production server 构建时核对权威 runtime method/path 清单，plugin、隐藏路由及 bind/call/apply 注册同样会触发失败；OpenAPI 仅验证公开契约，不作为安全边界。数据库权限仍必须由 release integration 证明 read 成功且 `INSERT`/`UPDATE`/`DELETE` 被拒绝。
