...
Acquisition commands do not reach persistence: existing runtime callers receive one stable 503 code and Admin mutations are structurally unregistered. Fastify's `onRoute` hook enforces the authoritative runtime route inventory, including plugin and hidden-route registrations; OpenAPI is contract evidence, not the security boundary. A closed runtime import graph, exact direct-import allowlists, mature ESLint restrictions, narrow Prisma adapter structure checks, deployment-template/seed checks, and runtime no-effect tests keep the shutdown from being widened by wiring or environment changes. Release integration must additionally prove the database role can read and that `INSERT`, `UPDATE`, and `DELETE` are denied; static checks do not substitute for that leased-database proof.
