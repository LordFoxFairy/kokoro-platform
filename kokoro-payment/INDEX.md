...
Acquisition commands do not reach persistence: existing runtime callers receive one stable 503 code and Admin mutations are structurally unregistered. Exact HTTP route/source inventories, a closed runtime import graph, direct-import allowlists, AST mutation/assembly checks, deployment-template/seed checks, and runtime no-effect tests keep the shutdown from being widened by wiring or environment changes; each detector class has a violating fixture. Release integration must additionally prove the database role can read and that `INSERT`, `UPDATE`, and `DELETE` are denied; static checks do not substitute for that leased-database proof.
