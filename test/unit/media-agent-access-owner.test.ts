...
describe("PostgresAgentImageAccessOwner", () => {
  it("resolves both opaque handles and returns only owner-frozen SiteRelease facts", async () => {
    ...
    expect(resolved).toMatchObject({ budgetSource: { kind: "agent_child",
      ...
      expectedParentRevision: 4n, expectedParentAllocationEpoch: 3n, unit: "credit" }, maximumCredit: 120n,
...
function row() {
  return Object.freeze({
    ...
    creditAgentRef: "agent:one", creditUnit: "credit", creditExpiresAt: "2026-07-31T13:00:00.000Z",
