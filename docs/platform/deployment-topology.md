...
本文约束 `kokoro-platform` 下平台子仓的 Docker、Kubernetes 和服务间调用方式。目标是让 `kokoro-user`、`kokoro-model`、`kokoro-credit`、`kokoro-payment` 都可以独立扩容，同时不因为从 Docker Compose 切到 Kubernetes 就改业务代码。
