...
All four Admin list resources are Site-scoped and strictly accept an optional `siteId`. Users/teams filter directly; memberships filter through team; service accounts filter through team or owner user. Relational rows return a projected `siteId`, and every filter is part of the Prisma query before `take: 100`; malformed queries never fall back to an unscoped list.
