...
All four Admin list resources are Site-scoped and strictly accept an optional `siteId`. Users/teams filter directly; memberships filter through team; service accounts require every present owner relation to resolve to the requested Site. Relational rows derive their projected `siteId` from those relations rather than the request, and cross-Site dual-owner service accounts are rejected. Every Site filter is part of the Prisma query before `take: 100`; malformed queries never fall back to an unscoped list.
